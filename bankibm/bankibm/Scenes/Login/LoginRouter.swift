...
class LoginRouter: NSObject, LoginRoutingLogic, LoginDataPassing {
    ...
    func routeToShowUserData(segue: UIStoryboardSegue?) {
        if let segue = segue {
            let destinationVC = segue.destination as! StatementListViewController
            var destinationDS = destinationVC.router!.dataStore!
            passDataToShowUserData(source: dataStore!, destination: &destinationDS)
        } else {
            let storyboard = UIStoryboard(name: "Main", bundle: nil)
            let destinationVC = storyboard.instantiateViewController(withIdentifier: "StatementListViewController") as! StatementListViewController
            var destinationDS = destinationVC.router!.dataStore!
            passDataToShowUserData(source: dataStore!, destination: &destinationDS)
    ...
    func navigateToShowUserData(source: LoginViewController, destination: StatementListViewController) {
        source.show(destination, sender: nil)
    }
    ...
    func passDataToShowUserData(source: LoginDataStore, destination: inout StatementListDataStore) {
