    ...
    static func request(_ jsonData: Data?, method: HTTPMethod, on resourcePath: String, headers: [String: String]?, completion: @escaping (RestResponse) -> Void) {
        ...
        let dataTask = session.dataTask(with: request) { (responseData: Data?, urlResponse: URLResponse?, error: Error?) in
            DispatchQueue.main.async {
                ...
                guard statusCode_2xx.contains(response.statusCode) else {
...
//    static func uploadFile(_ data: Data, method: HTTPMethod, on endpoint: String, headers: [String: String]?,  boundary: String, completion: @escaping (RestResponse) -> Void) {
//        var response = RestResponse(data: nil, statusCode: 0, result: .success)
//
//        // Create Url
//        let completePath = basePath + endpoint.addingPercentEncoding(withAllowedCharacters: CharacterSet.urlQueryAllowed)!
//        guard let url = URL(string: completePath) else {
//            print("\nErro na criação da url")
//            response.result = .failure(.urlError)
//            completion(response)
...
//        request.allHTTPHeaderFields = generateTokenHeader()
//
//        // Create Task
//        let dataTask = session.dataTask(with: request) { (responseData: Data?, urlResponse: URLResponse?, error: Error?) in
//            DispatchQueue.main.async {
...
//    static func generateTokenHeader() -> [String: String] {
//        let usuarioLogadoToken = ""
//        return ["Authorization": usuarioLogadoToken]
//    }
}
