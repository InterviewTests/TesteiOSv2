...
class LoginRouter: NSObject, LoginRoutingLogic, LoginDataPassing{
    ...
    func routeToStatement(segue: UIStoryboardSegue?) {
        if let segue = segue {
            ...
            let storyboard = UIStoryboard(name: Constants.Storyboards.statement, bundle: Bundle(for: StatementViewController.self))
            let destinationVC = storyboard.instantiateInitialViewController() as! StatementViewController
            var destinationDS = destinationVC.router!.dataStore!
            passDataToStatement(source: dataStore!, destination: &destinationDS)
