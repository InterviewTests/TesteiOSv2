...
protocol TesteAPICalls {
    ...
    func login(withLogin login: LoginModel, failure: @escaping ErrorBlock, completion: @escaping (UserModel?) -> Void)
...
extension TesteAPIManager: TesteAPICalls {
    ...
    func login(withLogin login: LoginModel, failure: @escaping ErrorBlock, completion: @escaping (UserModel?) -> Void) {
        request(.login(login: login), type: UserModel.self, failure: failure, completion: completion)
    }
